...
void *thread_function(void *var){
    ...
    char word_indexes[MAX_WORD_IN_A_PHASE], phase_buffer[MAX_WORD_IN_A_PHASE * (MAX_WORD_SIZE + 1)], md5_hash[2 * MD5_HASH_LENGTH + 1];
    char **found_element;
    ...
    start_index = parameters->skip_to_value + parameters->thread_id * ceil(parameters->full_interval_length / threads_count);
    stop_index = parameters->skip_to_value + min(parameters->full_interval_length, (parameters->thread_id + 1) * ceil(parameters->full_interval_length / threads_count));
...
int main(int argc, char **argv){
    ...
    char *hash = NULL;
    int found_hash_flag = 0, is_error = 0, word_array_size, base, full_interval_length, skip_to_value, hash_length, ret_val, i;
